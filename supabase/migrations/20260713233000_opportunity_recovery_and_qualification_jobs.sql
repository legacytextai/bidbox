...
-- A candidate may be present in only one queued/running recovery task. The
-- worker also performs a claim-time check because JSON payloads cannot be
-- protected by a conventional column unique constraint.
